<template>
  ...
  >
    <div class="relative w-full md:w-2/3 lg:w-1/2 flex flex-col gap-12 bg-white m-4 rounded-md p-4">
      ...
      <span class="text-center text-lg md:text-xl font-semibold">Update post</span>
      <form @submit.prevent="updatePost()" class="flex flex-col gap-8">
        ...
        <div class="w-full flex flex-col">
          <label class="font-semibold" for="">Content</label>
...
const updatePost = async () => {
  ...
  if (linkIsOk.value) {
    ...
    let request = await store.savePost(store.getUserId, post, store.selectedPost.id)
...
onMounted(() => {
  title.value = ((store.selectedPost === null) ? '' : store.selectedPost.title)
  link.value = ((store.selectedPost === null) ? '' : store.selectedPost.link)
  content.value = ((store.selectedPost === null) ? '' : store.selectedPost.content)
  originalTitle.value = ((store.selectedPost === null) ? '' : store.selectedPost.title)
  originalLink.value = ((store.selectedPost === null) ? '' : store.selectedPost.link)
  originalContent.value = ((store.selectedPost === null) ? '' : store.selectedPost.content)
})
</script>
