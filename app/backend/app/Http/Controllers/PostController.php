...
class PostController extends Controller
{
    ...
    function  search(Request $request)
    {
        ...
        $searchbyTag = Tag::select('post_id')
            ->join('post_tag', 'tags.id', '=', 'post_tag.tag_id')
            ->where('name', 'like', "%" . $search . "%")
            ->get();

        $tagResults = array_map(function ($searchbyTag) {
            $post = new PostResource(Post::find($searchbyTag['post_id']));
            return $post;
        }, [...$searchbyTag]);


        $searchByTitleOrContent = Post::where('title', 'like', "%" . $search . "%")
            ->orWhere('content', 'like', "%" . $search . "%")
            ->get();

        $titleOrContentResults = PostResource::collection($searchByTitleOrContent);
        

        $result =  array_merge($titleOrContentResults->all(), $tagResults);
        return $result;
    }
}
